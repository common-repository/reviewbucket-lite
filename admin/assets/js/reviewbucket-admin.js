( function($) {
	...
	$( '.reviewbucket-tab' ).on( 'click', function(e) {
 		e.preventDefault();
		...
		$('.reviewbucket-active').removeClass('reviewbucket-active').addClass('reviewbucket-hide');

		$($selector).removeClass('reviewbucket-hide').addClass('reviewbucket-active');

	} );

	$( '.reviewbucket-inner-tab' ).on( 'click', function(e) {

		e.preventDefault();

		let $selector = $(this).attr('href');

		$('.reviewbucket-inner-active').removeClass('reviewbucket-inner-active').addClass('reviewbucket-hide');

		$($selector).removeClass('reviewbucket-hide').addClass('reviewbucket-inner-active');
	...
	**************************************/
	...
	var $reviewtype = $('#reviewtype'),
		...
		$reviewbucketstyle 	= $("#reviewbucketstyle"),
		$reviewbucketstyle_wrap 	= $("#reviewbucketstyle_wrap"),
		$max_rows 	= $("#max_rows"),
		...
		$bg_color = $("#bg_color"),
		$reviewbucketcolumn = $("#reviewbucketcolumn"),
	...
	$reviewtype.on( 'change', function() {
		...
		if( $(this).val() == 'reviewbucket_google_place' ) {

			$reviewcat_wrap.hide()
			$gplaceinfo.show()
			$reviewbucketstyle_wrap.show()
			$max_rows_wrap.show()
		...
		}else if( $(this).val() == 'reviewbucket_custom_reviews' ) {
		...
		} else if( $(this).val() == 'reviewbucket_facebook_review' ){
			$reviewcat_wrap.hide()
			$gplaceinfo.hide()
			$reviewbucketstyle_wrap.show()
			$max_rows_wrap.show()
		...
		} else if ( $(this).val() == 'reviewbucket_yelp_review' ) {
	...
	$( '#scodegenerate' ).on( 'click', function() {
		...
		//
		if( $reviewbucketcolumn.length > 0 ) {
			$attr += ' column="'+$reviewbucketcolumn.val()+'"';
		}
		//
		if( $reviewbucketstyle_wrap.is(":visible") ) {
			$attr += ' style="'+$reviewbucketstyle.val()+'"';
		}
    ...
    $( '[name="reviewbucket_platform"]' ).on( 'change', function() {
	...
	**********************************************/
	...
	var $badgestype 	= $('#badgestype'),
		...
		$badgestype.on( 'change', function() {
			...
			if( val == 'reviewbucket_badges_slider' ) {
	...
	**************************************/
    ...
    $( '.get-google-review' ).on( 'click', function(e) {
    	...
    	$.ajax({
    		...
    		type: 'POST',
    		url: reviewbucketadminobj.admin_url,
    		data: {
    		...
    		var data = JSON.parse( data ),
				...
				if( reviewMessage.length || placeMessage.length ) {
					message.html('<p class="reviewbucket-alert">'+placeMessage+'<br>'+reviewMessage+'</p>');
				}
    ...
    $( '.reviewbucket-tab' ).on( 'click', function() {
